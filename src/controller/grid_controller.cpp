...
void GridController::setup(int x, int y, int w, int h)
{
    ...
    // fragShader.load("shader.frag");
    // vertShader.load("shader.vert");
...
void GridController::draw()
{
    ...
    // ofImage gridImage;
    // gridImage.allocate(grid.h, grid.w, OF_IMAGE_COLOR); // width, height

    // for (int y = 0; y < grid.h; y++)
    // {
    //     for (int x = 0; x < grid.w; x++)
    //     {
    //         ofColor col = grid.grid[y][x]->getCellColor();
    //         gridImage.setColor(x, y, col);
    //     }
    // }
    // gridImage.update();
    // gridImage.draw(displayPosX, displayPosY, gridImage.getWidth() * scaleX, gridImage.getHeight() * scaleY);
}
